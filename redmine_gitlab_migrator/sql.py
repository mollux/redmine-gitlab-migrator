...
REPAIR_ISSUE_LINKS = r"""
UPDATE issues SET
    title = regexp_replace(issues.title, '\\\#([0-9]+)','#\1', 'g'),
    description = regexp_replace(issues.description, '\\\#([0-9]+)','#\1', 'g'),
    title_html = '',
    description_html = ''
...
REPAIR_NOTE_LINKS = r"""
UPDATE notes SET
    note = regexp_replace(notes.note, '\\\#([0-9]+)','#\1', 'g'),
    note_html = ''
WHERE project_id={project_id};
"""
